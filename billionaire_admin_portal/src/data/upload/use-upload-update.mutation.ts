...
export interface IUploadUpdateVariables {
  variables: {
    id: string;
...
export const useUpdateUploadMutation = () => {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const updateMsg = t("common:successfully-updated")
  return useMutation(
    ({ variables: { id, input } }: IUploadUpdateVariables) =>
      Type.update(`${API_ENDPOINTS.ATTACHMENTS}/${id}`, input),
    {
      onSuccess: () => {
