...
import { allowedRoles, getAuthCredentials, hasAccess, setAuthCredentials } from "@utils/auth-utils";
  ...
  function onSubmit({ email, password }: FormValues) {
    login(
      ...
      },
      {
        onSuccess: ({ data }) => {
          if (data?.token) {
            if (hasAccess(allowedRoles, data?.user_data?.permission)) {
              setAuthCredentials(data?.token, data?.user_data?.permission, data?.user_data);
