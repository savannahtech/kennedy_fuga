...
const ShopForm = ({ initialValues }: { initialValues?: any }) => {
  ...
  async function onSubmit(values: FormValues) {
    ...
    // let logo, coverImage
    const coverImage: Attachment[] = await new Promise((resolve, reject) => {
      values.cover_image?.uploadFn(
        values.cover_image?.files, {
    ...
    const logo: Attachment[] = await new Promise((resolve, reject) => {
      values.logo?.uploadFn(
        values.logo?.files, {
          onSuccess: (data: Attachment[]) => {
            resolve(data);
          },
